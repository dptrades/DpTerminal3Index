            ...
            <div className="space-y-6 bg-gray-900/50 p-6 rounded-xl border border-gray-800 relative">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2">
                    ...
                    <CompactMetric
                        ...
                        subValue={
                            data.analysis.metrics.volumeDiff > 50 ? 'High Conviction' :
                            data.analysis.metrics.volumeDiff > 0 ? 'Normal' :
                            data.analysis.metrics.volumeDiff > -30 ? 'Low Conviction' :
                            'Weak Participation'
                        }
                    ...
                    <CompactMetric
                        label="Gamma Sqz"
                        value={
                            (data.analysis.metrics.gammaSqueeze?.score || 0) > 60 ? 'HIGH' :
                            (data.analysis.metrics.gammaSqueeze?.score || 0) > 30 ? 'MODERATE' : 'LOW'
                        }
                        subValue={
                            (data.analysis.metrics.gammaSqueeze?.score || 0) > 60 ? 'Elevated Exposure' :
                            (data.analysis.metrics.gammaSqueeze?.score || 0) > 30 ? 'Normal Exposure' : 'Low Exposure'
                        }
